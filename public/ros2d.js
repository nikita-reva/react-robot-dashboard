...
(createjs.Stage.prototype.globalToRos = function (a, b) {
  var c = (a - this.x) / this.scaleX,
    d = (this.y - b) / this.scaleY;
  return new ROSLIB.Vector3({ x: c, y: d });
}),
  (createjs.Stage.prototype.rosToGlobal = function (a) {
    var b = a.x * this.scaleX + this.x,
      c = a.y * this.scaleY + this.y;
    return { x: b, y: c };
  }),
  (createjs.Stage.prototype.rosQuaternionToGlobalTheta = function (a) {
    var b = a.w,
      c = a.x,
      d = a.y,
      e = a.z;
    return (
      (180 * -Math.atan2(2 * (b * e + c * d), 1 - 2 * (d * d + e * e))) /
      Math.PI
    );
  }),
  (ROS2D.ImageMap = function (a) {
    ...
    (this.pose = new ROSLIB.Pose({
      ...
      orientation: b.origin.orientation,
    })),
      (this.width = b.width),
  ...
  (ROS2D.OccupancyGrid = function (a) {
    ...
    var b = a.message,
      ...
      d = c.getContext('2d');
    (this.pose = new ROSLIB.Pose({
      position: b.info.origin.position,
      orientation: b.info.origin.orientation,
    })),
    ...
    for (
      var e = d.createImageData(this.width, this.height), f = 0;
      f < this.height;
      f++
    )
  ...
  (ROS2D.OccupancyGridClient = function (a) {
    ...
    e.subscribe(function (a) {
      var c = null;
      b.currentGrid &&
        ((c = b.rootObject.getChildIndex(b.currentGrid)),
        b.rootObject.removeChild(b.currentGrid)),
  ...
  (ROS2D.Grid = function (a) {
    ...
    for (var f = -b; b >= f; f++)
      e.moveTo(-b * c, f * c),
        ...
        e.moveTo(f * c, -b * c),
        e.lineTo(f * c, b * c);
    e.endFill(), e.endStroke(), createjs.Shape.call(this, e);
  }),
    ...
    if (
      ...
      this.graphics.setStrokeStyle(this.strokeSize),
        ...
        this.graphics.moveTo(
          b.poses[0].pose.position.x / this.scaleX,
          b.poses[0].pose.position.y / -this.scaleY
        );
      for (var c = 1; c < b.poses.length; ++c)
        this.graphics.lineTo(
          b.poses[c].pose.position.x / this.scaleX,
          b.poses[c].pose.position.y / -this.scaleY
        );
      this.graphics.endStroke();
    }
    createjs.Shape.call(this, this.graphics);
  }),
  (ROS2D.PathShape.prototype.setPath = function (a) {
    if ((this.graphics.clear(), null !== a && 'undefined' != typeof a)) {
      this.graphics.setStrokeStyle(this.strokeSize),
        this.graphics.beginStroke(this.strokeColor),
        this.graphics.moveTo(
          a.poses[0].pose.position.x / this.scaleX,
          a.poses[0].pose.position.y / -this.scaleY
        );
      for (var b = 1; b < a.poses.length; ++b)
        this.graphics.lineTo(
          a.poses[b].pose.position.x / this.scaleX,
          a.poses[b].pose.position.y / -this.scaleY
        );
      this.graphics.endStroke();
    }
  }),
  ...
  (ROS2D.PolygonMarker = function (a) {
    (a = a || {}),
      ...
      (this.lineColor =
        a.lineColor || createjs.Graphics.getRGB(0, 0, 255, 0.66)),
      (this.pointSize = a.pointSize || 10),
      (this.pointColor =
        a.pointColor || createjs.Graphics.getRGB(255, 0, 0, 0.66)),
      (this.fillColor =
        a.pointColor || createjs.Graphics.getRGB(0, 255, 0, 0.33)),
  ...
  (ROS2D.PolygonMarker.prototype.createLineShape = function (a, b) {
    ...
    return (
      c.addEventListener('mousedown', function (a) {
        null !== d.lineCallBack &&
          'undefined' != typeof d.lineCallBack &&
          d.lineCallBack(
            'mousedown',
            a,
            d.lineContainer.getChildIndex(a.target)
          );
      }),
      c
    );
  }),
  (ROS2D.PolygonMarker.prototype.editLineShape = function (a, b, c) {
    a.graphics.clear(),
  ...
  (ROS2D.PolygonMarker.prototype.createPointShape = function (a) {
    ...
    return (
      b.addEventListener('mousedown', function (a) {
        null !== c.pointCallBack &&
          'undefined' != typeof c.pointCallBack &&
          c.pointCallBack(
            'mousedown',
            a,
            c.pointContainer.getChildIndex(a.target)
          );
      }),
      b
    );
  }),
  (ROS2D.PolygonMarker.prototype.addPoint = function (a) {
    ...
    if (
      (c > 2 &&
        this.editLineShape(
          this.lineContainer.getChildAt(c - 2),
          this.pointContainer.getChildAt(c - 2),
          b
        ),
      c > 1)
    ) {
  ...
  (ROS2D.PolygonMarker.prototype.remPoint = function (a) {
    var b;
    b = a instanceof createjs.Shape ? this.pointContainer.getChildIndex(a) : a;
    var c = this.pointContainer.getNumChildren();
    2 > c ||
      (3 > c
        ? this.lineContainer.removeAllChildren()
        : (this.editLineShape(
            this.lineContainer.getChildAt((b - 1 + c) % c),
            this.pointContainer.getChildAt((b - 1 + c) % c),
            this.pointContainer.getChildAt((b + 1) % c)
          ),
          this.lineContainer.removeChildAt(b))),
      this.pointContainer.removeChildAt(b),
      this.drawFill();
  }),
  (ROS2D.PolygonMarker.prototype.movePoint = function (a, b) {
    var c, d;
  ...
  (ROS2D.PolygonMarker.prototype.splitLine = function (a) {
    var b, c;
    a instanceof createjs.Shape
      ? ((b = this.lineContainer.getChildIndex(a)), (c = a))
      : ((b = a), (c = this.lineContainer.getChildAt(b)));
    var d = this.pointContainer.getNumChildren(),
      e = this.pointContainer.getChildAt(b).x,
      f = this.pointContainer.getChildAt(b).y,
      g = this.pointContainer.getChildAt((b + 1) % d).x,
      h = this.pointContainer.getChildAt((b + 1) % d).y,
      i = (e + g) / 2,
      j = (f + h) / 2,
      k = new ROSLIB.Vector3({ x: i, y: -j }),
      l = this.createPointShape(k);
    this.pointContainer.addChildAt(l, b + 1), ++d;
    var m = this.createLineShape(
      l,
      this.pointContainer.getChildAt((b + 2) % d)
    );
  ...
  (ROS2D.PolygonMarker.prototype.drawFill = function () {
    ...
    if (a > 2) {
      ...
      for (var c = 1; a > c; ++c)
        b.lineTo(
          this.pointContainer.getChildAt(c).x,
          this.pointContainer.getChildAt(c).y
        );
      b.closePath(), b.endFill(), b.endStroke();
  ...
  (ROS2D.TraceShape.prototype.addPose = function (a) {
    var b = this.poses.length - 1;
    if (0 > b)
      this.poses.push(a),
        this.graphics.moveTo(
          a.position.x / this.scaleX,
          a.position.y / -this.scaleY
        );
    else {
      var c = this.poses[b].position.x,
        d = this.poses[b].position.y,
        e = a.position.x - c,
        f = a.position.y - d;
      e * e + f * f > this.minDist &&
        (this.graphics.lineTo(
          a.position.x / this.scaleX,
          a.position.y / -this.scaleY
        ),
  ...
  (ROS2D.TraceShape.prototype.popFront = function () {
    if (this.poses.length > 0) {
      ...
      for (var a = 1; a < this.poses.length; ++a)
        this.graphics.lineTo(
          this.poses[a].position.x / this.scaleX,
          this.poses[a].position.y / -this.scaleY
        );
    }
  }),
  ...
  (ROS2D.PanView.prototype.startPan = function (a, b) {
    (this.startPos.x = a), (this.startPos.y = b);
  }),
  (ROS2D.PanView.prototype.pan = function (a, b) {
  ...
  (ROS2D.Viewer.prototype.addObject = function (a) {
    this.scene.addChild(a);
  }),
  (ROS2D.Viewer.prototype.scaleToDimensions = function (a, b) {
    (this.scene.x =
      'undefined' != typeof this.scene.x_prev_shift
        ? this.scene.x_prev_shift
        : this.scene.x),
      (this.scene.y =
        'undefined' != typeof this.scene.y_prev_shift
          ? this.scene.y_prev_shift
          : this.scene.y),
      (this.scene.scaleX = this.width / a),
      (this.scene.scaleY = this.height / b);
  }),
  (ROS2D.Viewer.prototype.shift = function (a, b) {
  ...
  (ROS2D.ZoomView = function (a) {
    (a = a || {}),
      (this.rootObject = a.rootObject),
      (this.minScale = a.minScale || 0.001),
      this.rootObject instanceof createjs.Stage
        ? (this.stage = this.rootObject)
        : (this.stage = this.rootObject.getStage()),
  ...
  (ROS2D.ZoomView.prototype.startZoom = function (a, b) {
    (this.center.x = a),
      (this.center.y = b),
  ...
  (ROS2D.ZoomView.prototype.zoom = function (a) {
    this.startScale.x * a < this.minScale &&
      (a = this.minScale / this.startScale.x),
      this.startScale.y * a < this.minScale &&
        (a = this.minScale / this.startScale.y),
